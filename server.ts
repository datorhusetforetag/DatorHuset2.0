...
type FpsRtMode = "dlss" | "dlssFrameGen" | "base";
type FpsReportGame = {
  high1080: number;
  ultra1440: number;
  ultra4k: number;
  rt?: { fps: number; mode: FpsRtMode } | null;
};
type FpsReportProfile = Record<string, FpsReportGame>;

const FPS_REPORT_PROFILES: Record<string, FpsReportProfile> = {
  silver_speedster: {
...
const sanitizeFpsLabel = (value: unknown, maxLength = 80) => {
  if (typeof value !== "string") return "";
  return value.trim().slice(0, maxLength);
};

const clampFpsValue = (value: unknown, fallback = 0) => {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return Math.max(0, Math.round(fallback));
  return Math.max(0, Math.round(parsed));
};

const normalizeDlssFsrMode = (value: unknown, enabled: boolean) => {
...
const buildDefaultFpsSettings = () => {
  ...
  Object.entries(DEFAULT_FPS_MAP).forEach(([game, data]: any) => {
    ...
    Object.entries(data?.base || {}).forEach(([resolution, graphicsMap]: any) => {
      Object.entries(graphicsMap || {}).forEach(([graphics, fps]: any) => {
        entries.push({
          game,
          resolution,
          graphics,
...
const buildFpsSettingsFromReportProfile = (profile: FpsReportProfile) => {
  const entries: any[] = [];
...
const normalizeFpsProfileName = (value: string) =>
  value
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
...
const resolveFpsReportProfileKey = (productName: string) => {
  ...
  if (
    key.includes("all-black-all-out") ||
    key.includes("all-white-all-out") ||
    key.includes("all-black-white-all-out")
  ) {
    return "all_out_5080";
  }
  return null;
...
const buildReportedFpsSettingsForProductName = (productName: string) => {
...
const normalizeLegacyFpsSettings = (input: any) => {
  const entries: any[] = [];
  ...
  Object.entries(games).forEach(([game, gameSettings]: any) => {
    ...
    const resolutions =
      gameSettings?.resolutions && typeof gameSettings.resolutions === "object"
...
const sanitizeFpsEntries = (entries: any[]) => {
...
const sanitizeFpsSettings = (input: any, fallbackInput?: any) => {
  const fallback =
...
export async function createCheckoutSession(req: any, res: any) {
  try {
    const { cartItems, userEmail, fullName, phone, address, postalCode, city, addressId, shippingMethod } = req.body;
...
async function handleSuccessfulPayment(stripeSession: any) {
  ...
  const orderItems: Array<{ product_id: string; unit_price_cents: number; quantity: number }> = [];
  const emailItems: Array<{ name: string; quantity: number; total: number }> = [];
  let productTotal = 0;
...
export async function getAdminProducts(req: any, res: any) {
  try {
    ...
    const { data, error } = await supabase
      .from("products")
      .select(
        "id, name, slug, legacy_id, description, price_cents, currency, cpu, gpu, ram, storage, storage_type, tier, motherboard, psu, case_name, cpu_cooler, os, rating, reviews_count, dlss_multiplier, frame_gen_multiplier"
      )
...
/**
 * Admin: create product listing
 * POST /api/admin/products
 */
export async function createAdminProduct(req: any, res: any) {
  try {
    const { user, error: authError } = await getAuthUser(req);
    if (authError || !user) {
      return res.status(401).json({ error: authError || "Unauthorized" });
...
const getDefaultFpsSettingsForProduct = async (productId: string) => {
  try {
